...
use futures::{channel::mpsc, select, sink::SinkExt, stream::StreamExt, FutureExt};

use log::{error, info, trace};

use std::collections::hash_map::{Entry, HashMap};

use crate::{internal_messages::Event, state, tasks, types, ws_messages};
    ...
    'main: loop {
        ...
        match event {
            Event::NewConnection {
                ...
                trace!(
                    "event_broker: received new conn from conn {} player {} for room {}",
                    conn_id,
                    player_id,
                    room_name
                );
                    ...
                    if new_room {
                        match room_intermediate {
                            ...
                            Entry::Vacant(e) => {
                                ...
                                let new_room = state::SquareRoomState::new(room_name.as_str());
                                e.insert(new_room)
                            }
                        }
                    } else {
                ...
                trace!(
                    "event_broker: adding player {} to room {}",
                    player_id,
                    room_name
                );
                ...
                room.add_player(
                    ...
                    x,
                    y,
                    conn_outgoing_sender,
                );

                trace!(
                    "event_broker: saving room {} for conn {}",
                    room_name,
                    conn_id
                );
                ...
                task::spawn(tasks::conn_writer::run(
                    conn_id,
                    ws_sender,
                    disconnect_sender.clone(),
                    conn_outgoing_receiver,
                    shutdown_receiver,
                ));

                trace!("event_broker: broadcasting room state");
                let ref mut senders = room.player_senders;
                for player_sender in senders {
                    match player_sender
                        .sender
                        .send(ws_messages::RoomStateMessageEvent {
            ...
            Event::PlayerMove {
                ...
                trace!(
                    "event_broker: received move from conn {} for player {}",
                    from_id,
                    player_id
                );
                ...
                trace!(
                    "event_broker: updating player {} location in room {}",
                    player_id,
                    room_name
                );
